...
folder <- params$archive_folder
  ...
  past_events_query_params <- list(
    key=params$meetup_api_key, 
    sign=TRUE,
    page=1, # TODO: this is getting the first event, not the latest one 
    status="past"
  )
...
event_lag <- no_upcoming %>%
    inner_join(past_events %>% 
...
event_lag <- event_lag %>%
  ...
  mutate(created=as.POSIXct(created/1000, tz="UTC", origin="1970-01-01"),
         ...
         last_event_since=difftime(Sys.Date(), last_event, unit="days")
         
         ) %>%
  separate(timezone, c("region"), extra="drop", fill="left")

event_lag %>% select(chapter, last_event_since, last_event) %>% arrange(-last_event_since)
