...
get_rladies_members <- function (folder) {
...
for (n in 52:nrow(meetup_groups)) {
  events_url <- meetup_groups[n,]["events_url"]
  print(paste("Trying url:", events_url$events_url))
...
get_self_groups(params$archive_folder)
...
meetup_events_past_merge <- meetup_events_past %>% 
  ...
  mutate(
    ...
    created=as.POSIXct(created/1000, origin="1970-01-01"))
```
...
past_event_age <- meetup_events_past_merge %>%
  ...
  mutate(first_meetup_age=ceiling(as.numeric(difftime(as.Date("2017-12-01"), first_meetup, units="days"))),
...
past_rsvps_month_curr <- past_rsvps %>%
  filter(str_detect(local_date, "2017"))
