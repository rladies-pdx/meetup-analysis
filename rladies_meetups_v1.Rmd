...
## Member RSVP Proportions

What proportion of events has each member RSVP'ed to? What are the general trends in each group?
...
member_rsvps <- member_rsvps %>%
  mutate(
    ...
    group.name=ifelse(str_detect(group.name, "Warsaw"), "R-Ladies Warsaw/RUG", as.character(group.name))
    ) %>%
  group_by(group.name) %>%
  mutate(
...
```{r rsvp_proportion_plots}

# members who have rsvp'ed to at least 50% of events (groups with <10 events)

ggplot(member_rsvps_pct_summary %>% filter(rsvps_events_pct > .5 & num_events > 2 & num_events < 10),
       aes(x = reorder(group.name, members_pct), y = members_pct, fill = factor(pct_rnd))) +
  geom_col(position="stack") +
  coord_flip()

ggplot(member_rsvps_pct_summary %>% filter(rsvps_events_pct > .5 & num_events > 2 & num_events < 10),
       aes(x = reorder(group.name, num_members), y = num_members, fill = factor(pct_rnd))) +
  geom_col(position="stack") +
  coord_flip()

# members who have rsvp'ed to at least 25% of events (groups with 10+ events)

ggplot(member_rsvps_pct_summary %>% filter(rsvps_events_pct > .25 & num_events >= 10),
       aes(x = reorder(group.name, members_pct), y = members_pct, fill = factor(pct_rnd))) +
  geom_col(position="stack") +
  coord_flip()
