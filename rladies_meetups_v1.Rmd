...
```{r past_member_rsvps, eval=FALSE}
...
for (n in 322:nrow(meetup_events_past)) {
...
Determine number of members over time based on when members joined the group.

```{r historical_membership}
...
What groups have yet to have a first meetup?
...
How much time passes between the Meetup events? Are some groups regularly meeting every month? What is the typical time between Meetup groups?
...
Related to intervals above. In a given time period, how many meetups does each group have?
...
```{r past_rsvp_summary_plot, fig.width=10, fig.height=10}
...
```{r past_rsvp_freq_plot, fig.width=10, fig.height=10}
...
```{r member_rsvps_density_plot, fig.width=10, fig.height=10}
...
The plot below uses the inverse log to determine a scaled proportion to check RSVP's against. A group with a smaller number of events should expect more active members to have attended a higher proportion of those events (log(2) = 1 -> 1/1 = 100%, log(42) = 4 -> 1/4 = 25%)). This method also filters out groups with only one event (log(1) = 0 -> 1/0 = Inf).
...
ggplot(member_rsvps_pct_summary %>%
  ...
  geom_bar(aes(fill=factor(round(rsvps_events_pct,1))), stat="identity", position="stack") +
  theme_few() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Chapter", y="# Members", fill="% Events", title="Most Active Members by RSVP") +
  coord_flip()

# TODO scales pretty percent
```

### RSVP Proportion

To better understand the significance of of the "active" membership numbers computed above, we convert them into a proportion using the total number of members who have RSVPed.
...
ggplot(member_rsvps_pct_summary %>%
  ...
  theme_few() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Chapter", y="% RSVPed Members", fill="% Events", title="% Most Active Members by RSVP") +
  coord_flip()
...
```{r members_vs_rsvps_plot, fig.width=10, fig.height=10}
...
ggplot(meetup_rsvp_group_merge, aes(x=reorder(chapter, members), y=rsvp_members_diff_pct)) +
  ...
  theme_few() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Chapter", y="No RSVP Members - RSVPed Members", title="Members w/o RSVP vs RSVP Members") +
  coord_flip()
...
ggplot(rsvps_vs_members %>% filter(variable %in% c("rsvp_members_pct", "no_rsvp_members_pct")), 
  ...
  theme_few() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Chapter", y="% Members", fill="Member Type", title="% of Members") +
  coord_flip()
